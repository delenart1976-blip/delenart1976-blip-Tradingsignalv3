...
st.set_page_config(page_title="TradingSignalAI LIVE", page_icon="📈", layout="wide")
st.title("TradingSignalAI — LIVE DATA")
st.caption("Dati in tempo reale tramite API esterna. Nessun ordine automatico viene inviato.")
...
with st.sidebar:
    ...
    if not key_present:
        try:
            key_present = bool(st.secrets.get("MASSIVE_API_KEY", "").strip())
        except Exception:
            pass
...
if scan:
    if not key_present:
        st.error("MASSIVE_API_KEY mancante. Inseriscila in .env oppure in Streamlit Secrets.")
        st.stop()

    end = datetime.now(timezone.utc).date()
    start = end - timedelta(days=days)
    frames, errors = {}, {}
    progress = st.progress(0, text="Recupero dati live...")
    for index, (tf, multiplier, timespan) in enumerate(TIMEFRAMES, 1):
        try:
            ...
            if frame.empty:
                errors[tf] = "Nessun dato restituito"
            else:
                frames[tf] = frame
    ...
    if frames:
        ...
        if result["signal"] != "WAIT" and "15m" in result["frames"]:
            ...
            st.success("Segnale registrato nello storico live.")
else:
    st.info("Inserisci MASSIVE_API_KEY e premi SCANSIONA ORA per usare dati live reali.")

st.divider()
st.subheader("Storico segnali")
records = history()
st.dataframe(
    pd.DataFrame(records) if records else pd.DataFrame(columns=["timestamp", "asset", "signal", "score", "entry", "sl", "tp1", "tp2", "rr", "reason"]),
